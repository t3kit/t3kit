...
final class SimpleSiteHeaderCEPreviewListener extends StandardContentPreviewRenderer
{
    public function __invoke(PageContentPreviewRenderingEvent $event): void
    {
        ...
        if ($event->getRecord()['CType'] === 'simpleSiteHeader') {
            $itemContent = $event->getPreviewContent();
            $row = $event->getRecord();
            if ($row['simple_image']) {
                $itemContent .= $this->linkEditContent($this->getThumbCodeUnlinked($row, 'tt_content', 'simple_image'), $row);
