...
final class TextmediaCEPreviewListener extends StandardContentPreviewRenderer
{
    public function __invoke(PageContentPreviewRenderingEvent $event): void
    {
        ...
        if ($event->getRecord()['CType'] === 'textmedia') {
            $itemContent = $event->getPreviewContent();
            $row = $event->getRecord();
            if ($row['advanced_media']) {
                $itemContent .= $this->linkEditContent($this->getThumbCodeUnlinked($row, 'tt_content', 'advanced_media'), $row);
